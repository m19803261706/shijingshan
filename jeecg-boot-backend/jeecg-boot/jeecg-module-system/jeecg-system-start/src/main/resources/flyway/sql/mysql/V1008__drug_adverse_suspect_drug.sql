...
-- 药品不良反应上报系统 - 怀疑药品子表
-- 版本: V1008
-- 描述: 创建怀疑药品子表 drug_adverse_suspect_drug
-- 作者: TC Agent
-- 创建时间: 2025-12-19
-- 参考: 国家药品不良反应/事件报告表 - 怀疑药品栏
-- ========================================

-- 创建怀疑药品子表（一对多关系，一个报告可有多个怀疑药品）
CREATE TABLE IF NOT EXISTS `drug_adverse_suspect_drug` (
  ...
  KEY `idx_batch_no` (`batch_no`) COMMENT '批号索引',
  CONSTRAINT `fk_suspect_drug_report` FOREIGN KEY (`report_id`) REFERENCES `drug_adverse_report` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='药品不良反应报告-怀疑药品子表';

-- 添加表注释
ALTER TABLE `drug_adverse_suspect_drug` COMMENT = '药品不良反应报告-怀疑药品子表 - 存储报告中涉及的怀疑药品信息，支持多行录入';
