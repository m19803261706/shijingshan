 ...
 */
@Mapper
public interface DrugCommonSuspectMapper extends BaseMapper<DrugCommonSuspect> {
     ...
     */
    @Select("<script>" +
            "SELECT * FROM drug_common_suspect " +
            "WHERE 1=1 " +
            ...
            "LIMIT 50" +
            "</script>")
    List<DrugCommonSuspect> searchByKeyword(@Param("keyword") String keyword);
     ...
     */
    @Select("<script>" +
            "SELECT COUNT(*) FROM drug_common_suspect " +
     ...
     * @return 更新的记录数
     */
    @Update("UPDATE drug_common_suspect SET use_count = use_count + 1, last_used_time = NOW() WHERE id = #{id}")
