...
import org.jeecg.modules.adverse.mapper.DrugCommonConcomitantMapper;
...
@Slf4j
@Service
public class DrugCommonConcomitantServiceImpl extends ServiceImpl<DrugCommonConcomitantMapper, DrugCommonConcomitant>
        implements IDrugCommonConcomitantService {

    @Override
    public List<DrugCommonConcomitant> searchByKeyword(String keyword) {
        return baseMapper.searchByKeyword(keyword);
    }

    @Override
    public boolean saveIfNotExists(DrugCommonConcomitant drug) {
        // 检查是否已存在
        ...
        log.info("保存新的常用并用药品配置: genericName={}", drug.getGenericName());
