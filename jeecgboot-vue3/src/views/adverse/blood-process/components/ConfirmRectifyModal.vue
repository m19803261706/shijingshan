...
import {
  getBloodProcessRectifyHistory,
  bloodProcessConfirmRectify,
  bloodProcessRejectRectify,
} from '/@/api/adverse/bloodProcess';
...
const [registerModal, { closeModal, setModalProps }] = useModalInner(async (data) => {
  ...
  if (data?.record) {
    ...
    // 加载最新整改记录
    const rectifyList = await getBloodProcessRectifyHistory(data.record.id);
...
async function handleSubmit() {
  try {
    ...
    if (formState.result === 'approve') {
      await bloodProcessConfirmRectify({
        id: reportData.value.id,
        comment: formState.comment,
      });
      createMessage.success('整改确认通过，报告已结案');
    } else {
      await bloodProcessRejectRectify({
        id: reportData.value.id,
        comment: formState.comment,
      });
