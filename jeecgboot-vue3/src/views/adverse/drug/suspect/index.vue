 ...
 * 常用怀疑药品配置 - 列表页
 * <p>
 * 提供常用怀疑药品的列表展示、搜索、新增、编辑、删除、导入导出功能
 * </p>
 *
 * @author TC Agent
 * @since 2025-12-19
...
<template>
  <div class="p-4">
    ...
    <!-- 新增/编辑弹窗 -->
    <SuspectModal @register="registerModal" @success="reload" />
  </div>
</template>
...
import { columns, searchFormSchema } from './suspect.data';
import {
  getSuspectList,
  deleteSuspect,
  deleteSuspectBatch,
  getExportUrl,
  getImportUrl,
} from '@/api/adverse/suspect';
import SuspectModal from './SuspectModal.vue';

// 页面标题
defineOptions({ name: 'DrugCommonSuspectList' });
...
const { tableContext } = useListPage({
  designScope: 'adverse-suspect-list',
  tableProps: {
    title: '常用怀疑药品配置',
    api: getSuspectList,
    columns: columns,
...
async function handleBatchDelete() {
  ...
  try {
    await deleteSuspectBatch(selectedRowKeys.value.join(','));
