<template>
  <div class="device-rectify-list">
    ...
    <!-- 整改表单弹窗 -->
    <DeviceRectifyFormModal @register="registerFormModal" @success="handleSuccess" />
  </div>
</template>

<script lang="ts" name="clinic-rectify-device-list" setup>
/**
 * 医疗器械不良事件整改列表
 * @description 临床科室人员查看和提交医疗器械不良事件整改
 * @author TC Agent
 * @since 2025-12-19
 * @see Issue #71
 */
...
import { getDeviceRectifyList, getDeviceCurrentRectify } from '/@/api/adverse/deviceClinicRectify';
import DeviceReportDetailModal from '../../clinic-audit/device/components/DeviceReportDetailModal.vue';
import RectifyHistoryModal from '../../device-process/components/RectifyHistoryModal.vue';
import DeviceRectifyFormModal from './DeviceRectifyFormModal.vue';
...
const { tableContext } = useListPage({
  designScope: 'clinic-rectify-device',
  tableProps: {
    title: '医疗器械不良事件待整改列表',
    api: getDeviceRectifyList,
    columns: columns,
    size: 'small',
    formConfig: {
    ...
    afterFetch: async (records) => {
      ...
      for (const record of records) {
        try {
          const rectify = await getDeviceCurrentRectify(record.id);
          if (rectify) {
