...
import { bloodAuditPass, bloodAuditReject } from '/@/api/adverse/bloodAudit';
...
async function handleSubmit() {
  try {
    ...
    if (formState.result === 'pass') {
      await bloodAuditPass(params);
      createMessage.success('审核通过');
    } else {
      await bloodAuditReject(params as { id: string; comment: string });
