...
import { deviceAuditPass, deviceAuditReject } from '/@/api/adverse/deviceAudit';
...
async function handleSubmit() {
  try {
    ...
    if (formState.result === 'pass') {
      await deviceAuditPass(params);
      createMessage.success('审核通过');
    } else {
      await deviceAuditReject(params as { id: string; comment: string });
