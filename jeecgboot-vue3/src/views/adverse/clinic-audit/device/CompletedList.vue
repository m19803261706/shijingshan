...
<script lang="ts" name="clinic-audit-device-completed" setup>
/**
 * 医疗器械不良事件已审核列表页面
 * @description 临床科室已审核医疗器械不良事件报告列表，显示审核结果和流转历史
 * @author TC Agent
 * @since 2025-12-19
 * @see Issue #69
 */
...
import { completedColumns, completedSearchFormSchema } from './deviceAudit.data';
import { getDeviceCompletedAuditList } from '/@/api/adverse/deviceAudit';
...
const { tableContext } = useListPage({
  designScope: 'clinic-audit-device-completed',
  tableProps: {
    title: '已审核报告列表',
    api: getDeviceCompletedAuditList,
